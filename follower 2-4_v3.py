...
sitl = None
vehicle = connect('tcp:0.0.0.0:5790', wait_ready=True)
...
def on_message(client,userdata,msg): 
    ...
    dest = LocationGlobal(m_in["late"], m_in["longe"]+0.0005, m_in["alte"])
...
while True:
    ...
    for variable in ["late", "longe", "alte","timestamp"]:
        ...
        print('3rd Leader GPS POS=',vehicle.location.global_relative_frame)
    data_out=json.dumps(leader_gps)  
    client.publish("test/3",data_out)
    
    cur_time=time.time()
    
    time.sleep(5)
    if ((cur_time-timestamp)>65):
        print("Disconnected from backup leader 1")
        
        client.unsubscribe("test/2")
        client.subscribe("test/1")    
        print("Searching for leader")
        new_time=time.time()
        if ((new_time-timestamp)>40):
