...
sitl = None
vehicle = connect('tcp:0.0.0.0:5800', wait_ready=True)
...
def on_message(client,userdata,msg): 
    ...
    dest = LocationGlobal(m_in["late"]+0.0005, m_in["longe"]+0.0005, m_in["alte"])
    print('I am at GPS POS =',vehicle.location.global_relative_frame)
    print ("However i am Going to: %s" , dest)
    global timestamp 
    timestamp=time.time()
...
later=vehicle.location.global_relative_frame.lat
longer=vehicle.location.global_relative_frame.lon
alter=vehicle.location.global_relative_frame.alt
...
while True:
    ...
    vehicle.commands.next=0 
    cur_time=time.time()
    if ((cur_time-timestamp)>65):
        print("All leaders have died ")
        print("No Captain Available")
        client.unsubscribe("test/3")
        ...
        new_time=time.time()
        if ((new_time-timestamp)>50):
            print("Stopping Mission")
            print('Vehicle Returning To Launch Site')
            ...
            while True: 
                ...
                print('My Position=',vehicle.location.global_relative_frame) 
    ...
    #client.subscribe("test/follow-me")
